...
const DEFAULT_PROJECT_NAME = 'Sbeamer Gabps'
const DEFAULT_PROJECT_ID = 'sbeamer_gapbs'
const DEFAULT_MODEL = 'mlr'
...
const lineChartOptions = {
    scales: {
        ...
        yAxes: [{
            ...
            scaleLabel: {
                display: true,
                labelString: 'Energy Consumption in CPU (in Joules)',
                fontSize: 14,
            },
        }]
    },
...
function returnTableData(real, forecasted) {
    ...
    for(var i = 0; i < forecasted.length; i++) {
        ...
        tableData.push({
            ...
            'energy_consumption': forecasted[i]['y'].toExponential(3) + ' Joules',
                    ...
                    <MDBFormInline className="md-form m-0">
                        <MDBDropdown>
                            ...
                            <MDBDropdownMenu basic>
                                ...
                                <MDBDropdownItem onClick={(param) => props.updateProjectData('Sbeamer Gabps','sbeamer_gapbs')}>Sbeamer Gabps</MDBDropdownItem>
                            </MDBDropdownMenu>
                        </MDBDropdown>
...
// The Evolution Panel
const EnergyEvolutionPanel = props => {
        ...
        <MDBRow className="mb-4">
            <MDBCol md="12" lg="12" className="mb-12">
                ...
                <MDBCardHeader className="sdk4ed-color">
                    Energy Consumption Evolution <MDBTooltip domElement tag="span" placement="top"><i className="fa fa-info-circle" aria-hidden="true"></i><span>Interactive plot showing the past (green) and the predicted (red) Energy Consumption evolution</span></MDBTooltip>
                </MDBCardHeader>
                <MDBCardBody>
...
const EnergyForecasterPanel = props => {
    ...
    return (
        <MDBRow className="mb-4">
            <MDBCol md="12" lg="6" className="mb-12">
                <MDBCard className="mb-12">
                <MDBCardHeader className="sdk4ed-color">
                    Energy Consumption Forecasting <MDBTooltip domElement tag="span" placement="top"><i className="fa fa-info-circle" aria-hidden="true"></i><span>Interactive plot focusing solely on the forecast, giving a more fine-grained view</span></MDBTooltip>
                </MDBCardHeader>
                <MDBCardBody>
            ...
            <MDBCol md="12" lg="6" className="mb-12">
                <MDBCard className="mb-12">
                <MDBCardHeader className="sdk4ed-color">
                    Energy Consumption Forecasting Details <MDBTooltip domElement tag="span" placement="top"><i className="fa fa-info-circle" aria-hidden="true"></i><span>Complementary table comprising the detailed results and trends of the forecasts</span></MDBTooltip>
                </MDBCardHeader>
                <MDBCardBody>
                    <p align="left" style={{color:'#548235'}}>Current Energy Consumption: <span style={{color:'#000000'}}>{props.myground_truth_data[props.myground_truth_data.length - 1]['y']} Joules</span></p>
...
class EnergyDashPage extends React.Component {
    ...
    updateProjectData = (project_name, project_id) => {
        ...
        this.getData(SERVER_IP+'/ForecasterToolbox/EnergyForecasting',DEFAULT_HORIZON,project_id,DEFAULT_MODEL,GROUND_TRUTH,TEST_MODELS)
        .then(resp => {
            if (resp.status === 200) {
    ...
    updateForecastedData = (new_algorithm, new_horizon) => {
        ...
        // Code for fetching json data from API
        this.getData(SERVER_IP+'/ForecasterToolbox/EnergyForecasting',new_horizon,project_id,new_algorithm,GROUND_TRUTH,TEST_MODELS)
        .then(resp => {
            if (resp.status === 200) {
                console.log("Data received")
    ...
    componentDidMount(){
        ...
        // If project is selected from the project panel
            ...
            if (selectedProjectSession['forecaster'] !== ''){
                ...
                var selectedProjectID = selectedProjectSessionMeta['name_in_energy_toolbox']
            // If selected project has no forecaster metadata filled then use git url to extract project ID
            }else{
                // get project_id as project_name
                var selectedProjectID = (selectedProjectURL.replace('.git','').split('/'))[selectedProjectURL.split("/").length - 1]
            } 
